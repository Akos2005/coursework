...
def prepare_for_rnn(df, timesteps=110):
    import numpy as np
    features = 60
    X_list, y_list = [], []

    for seq_id, group in df.groupby("sequence_id"):
        arr = group.drop(columns=["sequence_id","performer","gesture_type","session"]).to_numpy()
        if arr.shape[0] >= timesteps:
            arr = arr[:timesteps, :]
        else:
            pad = np.zeros((timesteps - arr.shape[0], features))
            arr = np.vstack([arr, pad])
        X_list.append(arr)
        y_list.append(group["gesture_type"].iloc[0])

    X_all = np.stack(X_list)
    y_series = pd.Series(y_list).astype("category")
    y_all = y_series.cat.codes.to_numpy()
    gesture_mapping = dict(enumerate(y_series.cat.categories))

   
    if len(X_all) < 6: 
        print("Túl kevés adat, egyszerű splitre állítva")
        X_train = X_all
        y_train = y_all
        X_val = X_test = np.empty((0, timesteps, features))
        y_val = y_test = np.empty((0,))
    else:
    ...
    return X_train, X_val, X_test, y_train, y_val, y_test, gesture_mapping
